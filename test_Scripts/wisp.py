...
class guiProj:
    def __init__(self, master):
        ...
        def submit_pref():
            ...
            def populate(frame):
                ...
                for iter_ in range(int(get_pref_no)):
                    ...
                    self.label_pref[iter_] = Label(frame, textvariable=self.text_pref[iter_],background="#f0fff0")
                    self.label_pref[iter_].grid(row=5+iter_+1, column=0, columnspan=1, sticky=W+N)

                    # entry widget
                    self.entry_pref[iter_] = Entry(frame,background="#f0ffff")
            ...
            canvas = tk.Canvas(master, borderwidth=0, background="#f0f8ff")
            frame = tk.Frame(canvas,background="#f0fff0")
            ...
            self.show_map_button = Button(master, text="show map",command=self.show_map,background="#ffc0cb")
        ...
        self.use_default_sec = Button(master, text="use default secrets",command=def_sec,background="#87ceeb")
        self.use_default_sec.grid(row=5,column=0,columnspan=1, sticky=W+E)
        # again button thing
        self.submit_pref_buttton = Button(master, text="submit",command=submit_pref,background="#afeeee")
